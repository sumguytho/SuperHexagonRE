...
windows as well since they share mostly the same codebase when it comes to game
...
The values, variable types and their offsets are correct but their decription might
...
graphicsclass::print along with strings being printed and their colours
 - other/cull_redundant_prints.py - a python script used to cull redundant string
...
projects, enums are the same for both versions of the game
...
the hyper stages from stage select isn't an exception from this. When you launch some hyper stage
...
time in the part of game logic where:
...
by that time your game timer and number of waves generated are what it needs to be to start getting
...
When you start a hyper stage from stage select the switch_to_hypermode flag is set which makes the game adjust the game
timer (add minute before doing game behaviour, substitute it right after the game behaviour code) and
also the number of waves generated is set to 0x33 (51) (and maybe some other stuff which I've missed that
doesn't seem to affect gameplay anyway).
...
random number which is then used as a third varible to determine the things mentioned above, the
first two being current value of game timer and number of waves generated. As an example:
...
the game may generate, what I don't show here is that there also conditions for the number of waves
...
There sure is more to this than just changing 2 variables but it's the overall weirdness that simply changing
the value of a variable that holds the number of the current stage can launch the game:

file://change_stage

If you set jiffies_after_game_is_lost to 0 after the game is lost, the game will continue and you will
even retain the current_time:
...
the game suddenly starts acting as if it were last stage. It also checks whether colour palette is that
of last stage and if it isn't it does following:

 - set colour palette to the palette of last stage
 - clear all obstacles
 - do a zoomout + spin
 - flash the screen for 10 game time jiffies
 - postpone obstacle generation by 180 game jiffies (3 seconds)

There is also a grace period of 13.(3) after you've reached the last stage when the
camera doesn't spin.
...
For some time I couldn't figure out how to change text colour for the stage in
...
it's done twice. First, the game dips into the game state to pick the stage name to draw
and then it does so again to determine the colour for the said labels which should already
be known since the game has just drawn stage name.
...
This example is my way of saying that possibilities of missing something out when
...
deviceclass meanwhile the actual keyboard polling is probably done somewhere in SDL2.
...
visual effect and goes back to 0 once the visual effect finalizes.
...
100 and stays at this value until you either restart the stage or go to stage select, after which to goes to 0
again. I imagine this variable is used as a state of score screen, can't tell for sure though.
...
I imagine this one is used to advance colours on hexagonest stage, it goes 0 to 255.
...
scenes (at stage select, while playing, at score screen) and I would think that these values are
used by game to differentiate between the states of the game if it wasn't for the fact that those
values are different between game restarts (each "game state value" is like give one or take one
between restarts).
...
The time in jiffies before the next wave is generated.

---

##### waves_generated

See explanations earlier. A wave may be comprised of multiple obstacles.

---

##### float2

The rate at which float1 decreases during the game.

---

##### obstacle_movespeed

Self evident. The game has no protection against this variable being set to value the game doesn't expect.
If you set it too low the game will crash due to obstacles overflowing whatever buffer they are written
to when created. Demo:
...
The state of something, changes from -40 to 100. It affects obstacle creation in some way.
...
Set to 1 once a collision is detected. It's actually a float and there several other values it's
set to, although they all seem to mean the same: game over.
...
Set to 1 for hexagonest and its hypermode, makes the field switch colours on adjacent obstacle lanes:
...
The time in game jiffies before the next wave is generated.
...
These flags control whether you can start the corresponding hypermode.
...
Didn't look into this one enough, set to 1 either when secret ending is shown or when the
...
The amount of time you have to spend on a stage in game jiffies to reach a certain rank. The are
...
(it's done through other variables from game structure).
